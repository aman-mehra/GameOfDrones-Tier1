...
class BaselineRacer(object):
    def __init__(self, drone_name = "drone_1", viz_traj=True, viz_traj_color_rgba=[1.0, 0.0, 0.0, 1.0], viz_image_cv2=True):
        ...
        self.drone_2_pose_live = None

        self.MAX_NUMBER_OF_GETOBJECTPOSE_TRIALS = 20 # see https://github.com/microsoft/AirSim-NeurIPS2019-Drone-Racing/issues/38
    ...
    def initialize_two_drone(self):
        for drone_name in ["drone_1","drone_2"]:
            self.airsim_client.enableApiControl(vehicle_name=drone_name)
            self.airsim_client.arm(vehicle_name=drone_name)

            # set default values for trajectory tracker gains 
            self.traj_tracker_gains = airsim.TrajectoryTrackerGains(kp_cross_track = 5.0, kd_cross_track = 0.0, 
            ...
            self.airsim_client.setTrajectoryTrackerGains(self.traj_tracker_gains, vehicle_name=drone_name)
            time.sleep(0.2)
    ...
    def takeoff_with_moveOnSpline(self, takeoff_height = 1.0):
        for drone_name in ["drone_1"]:
            start_position = self.airsim_client.simGetVehiclePose(vehicle_name=drone_name).position
            ...
            self.airsim_client.moveOnSplineAsync([takeoff_waypoint], vel_max=15.0, acc_max=5.0, add_position_constraint=True, add_velocity_constraint=False, 
                add_acceleration_constraint=False, viz_traj=self.viz_traj, viz_traj_color_rgba=self.viz_traj_color_rgba, vehicle_name=drone_name).join()
    ...
    def fly_through_all_gates_at_once_with_moveOnSpline(self):
        ...
        if self.level_name == "Building99_Hard":
            vel_max = 4.0
            acc_max = 1.0
        print("&&&$$$$HERE!)!)!)!1010101010101101000000000000")
        while(True):
            self.airsim_client.moveOnSplineAsync([gate_pose.position for gate_pose in self.gate_poses_ground_truth], vel_max=vel_max, acc_max=acc_max, 
                add_position_constraint=True, add_velocity_constraint=False, add_acceleration_constraint=False, viz_traj=self.viz_traj, viz_traj_color_rgba=self.viz_traj_color_rgba, vehicle_name=self.drone_name)
            time.sleep(0.1)
            print("ye")
            self.airsim_client.clearTrajectory(self.drone_name)
    ...
    def odometry_callback(self):
        ...
        self.drone_2_pose_live = self.airsim_client_odom.simGetVehiclePose(self.drone_names[1]).position
        # d1_abs = self.airsim_client_odom.simGetVehiclePose(self.drone_names[0]).position

        # self.drone_2_pose_live = airsim.Vector3r(d2_abs.x_val-d1_abs.x_val,d2_abs.y_val-d1_abs.y_val,d2_abs.z_val-d1_abs.z_val)
