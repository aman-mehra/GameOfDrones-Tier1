...
class Actor(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.lstm = nn.LSTM(state_dim, 32, 2,batch_first=True, dropout=0.2)
        self.l1 = nn.Linear(32, 16)
        self.bn1 = nn.BatchNorm1d(num_features=16)
        self.l2_a = nn.Linear(16, action_dim-1)
        self.l2_b = nn.Linear(16, 1)


    # x --> batch*seq*features
    def forward(self, x):
        lstm_out, _ = self.lstm(x)
        inter1 = F.relu(self.bn1(self.l1(lstm_out[:,-1,:])))
        vec = torch.tanh(self.l2_a(inter1))
        vel = torch.sigmoid(self.l2_b(inter1))  
...
class Critic(nn.Module):
    ...
    def __init__(self, state_dim, action_dim):
        ...
        self.l1 = nn.Linear(state_dim + action_dim, 48)
        ...
        # Q2 architecture
        self.l4 = nn.Linear(state_dim + action_dim, 48)
        self.bn3 = nn.BatchNorm1d(num_features=48)
        self.l5 = nn.Linear(48, 24)
        self.bn4 = nn.BatchNorm1d(num_features=24)
        self.l6 = nn.Linear(24, 1)


    def forward(self, x, u):
        xu = torch.cat([x[:,-1,:], u], 1)
...
class ReplayBuffer(object):
    ...
    def load_buff(self):
        my_file = open("./analysis/replay_buffer-lstm","rb")
        return pickle.load(my_file)

    def dump_buff(self):
        my_file = open("./analysis/replay_buffer-lstm","wb+")
    ...
    def sample(self, batch_size):
        ...
        for i in ind: 
            ...
            states.append(s)
            next_states.append(s_.reshape(1,4,52))
            actions.append(a.reshape(1,4))
        ...
        return np.array(states), next_states, actions , np.array(rewards).reshape(-1, 1), np.array(dones).reshape(-1, 1)
...
class TD3(object):
    ...
    def select_action(self, state, noise=0.1):
        ...
        state = torch.FloatTensor(state.reshape(1, 4, -1)).to(device)
    ...
    def save(self, episode_number, filename = ""):
        torch.save({
            "ep": episode_number,
            ...
            },"./saves/model-lstm"+filename+".pth")

    def load(self, filename="best_avg", directory="./saves"):
        print("Loading model ....")

        checkpoint = torch.load("./saves/model-lstm.pth")
        ...
        if done: 
            ...
            if total_timesteps != 0:
                ...
                if best_avg < avg_reward:
                    ...
                    agent.save(episode_num,"-best_avg")
                else:
                    agent.save(episode_num)
...
if __name__ == "__main__":
    ...
    log_file = open("analysis/log-lstm.txt", "a+")
