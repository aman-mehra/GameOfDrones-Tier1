...
class BaselineRacerGTP(BaselineRacer):
    ...
    def calculate_velocity(self, action = [0, 0, 0, 0],stall=False):
        ...
        # action[3] += 1
        # print(action[3])
        # diff = (1 + 8 * action[3])*(diff) + noise

        diff = (3+9*(action[3]-0.2))*(diff+0.12*del_v)*(1-int(stall))
    ...
    def update_and_plan(self,delta_v=[0.,0.,0.,1.],stall=False):
        ...
        if self.gates_complete == len(self.gate_poses_ground_truth):
            ...
            return next_state,reward,True,"finished debug"
        ...
        return next_state, reward, done, "debug" # next_state,reward,done,"debug string"
    ...
    def step(self,action,stall = False):
        ...
        if self.time - self.last_gate_change_time >= 40000:
            return self.construct_feature_vector(),0,True,"Stuck"
    ...
    def reset(self):
        ...
        return self.construct_feature_vector()
